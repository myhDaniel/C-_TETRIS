...
private:
	...
	int y;
public:
	S_Block(int _x,int _y,COLORREF c,int rot):Block(4,c,rot)
	{
		x=_x;
		y=_y;
	}
	void draw(int size)
	{
		...
		else if (rotationIndex==2)
		{
			...
			fillrectangle(x,y+size,x+size,y+2*size);
			fillrectangle(x+size,y+size,x+2*size,y+2*size);
			fillrectangle(x+size,y+2*size,x+2*size,y+3*size);
		}		
	}
	void Clear(int size)
	{
		...
		else if (rotationIndex==2)
		{
			...
			clearrectangle(x,y+size,x+size,y+2*size);
			clearrectangle(x+size,y+size,x+2*size,y+2*size);
			clearrectangle(x+size,y+2*size,x+2*size,y+3*size);
		}	
	}
	...
	void rotate(int size,int (*blockMap)[15],int _x,int _y,int &r)
	{
		...
		{
			S_Block S(_x,_y,0,2);
			if (S.ifEmpty(_x,_y,size,blockMap))
				r=2;
		}
		else if (rotationIndex==2)
		{
